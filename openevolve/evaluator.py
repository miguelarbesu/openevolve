...
class Evaluator:
        ...
        try:
            ...
            try:
                ...
                async def run_stage3():
                    loop = asyncio.get_event_loop()
                    # Check if evaluate_stage3 accepts llm_ensemble
                    sig = inspect.signature(module.evaluate_stage3)
                    if "llm_ensemble" in sig.parameters:
                        if asyncio.iscoroutinefunction(module.evaluate_stage3):
                            return await module.evaluate_stage3(
                                program_path, self.llm_ensemble
                            )
                        else:
                            return await loop.run_in_executor(
                                None,
    ...
    async def _llm_evaluate(
        ...
        try:
            ...
            feature_dimensions = (
                self.database.config.feature_dimensions if self.database else []
            )
